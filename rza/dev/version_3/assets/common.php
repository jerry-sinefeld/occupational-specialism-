...
function reg_user($conn,$post)
{
        ...
        $hpswd = password_hash($post["password"], PASSWORD_DEFAULT); /*hashes password using prebuilt library in php// TODO: POTENTIAL CHANGE NEEDED
...
function login($conn,$post){
        $sql = "SELECT * FROM user WHERE username = ?"; //select everything from the user table where username = the entered username// TODO: POTENTIAL CHANGE NEEDED
        $stmt = $conn->prepare($sql);//prepare the sql for data
...
function only_user($conn, $username)
{
        ...
        $stmt->bindParam(1, $username);// TODO: POTENTIAL CHANGE NEEDED //we are binding the data from our form to a sql statement this makes it more secure from an sql attack and makes it unlikely for people to hijack an sql statement// TODO: POTENTIAL CHANGE NEEDED
