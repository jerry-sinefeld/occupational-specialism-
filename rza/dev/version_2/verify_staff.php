...
$employ_id = $_SESSION["employ_id"] ?? null; // if this value exists use it otherwise null
if (!$employ_id) {
    $_SESSION["usermessage"] = "Verification error: Please register first.";
    header("location: staff_reg.php"); //checks if there is a doc ID in the session and if there isn't it redirects you to the register page
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["request_code"])) {
        $auth_code = code_request($conn, $employ_id);
        if ($auth_code) {
    ...
    if (isset($_POST["submit_verification"])) {
        ...
        if (auth($conn, $submitted_code)) {//authenticates
            if (activate_engin($conn, $employ_id)) {//sets engineer as active
