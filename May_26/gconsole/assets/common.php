...
function new_console($conn, $post)
{
    try {//we are doing a prepared statement where we send the statement and the data separately to prevent sql injections
        ...
        $stmt->bindParam(3, $post['sign']); //we are binding the data from our form to an sql statement this makes it more secure from an sql attack and makes it unlikely for people to hijack an sql statement
