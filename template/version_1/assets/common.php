...
function reg_user($conn,$post)
{
    try{
        ...
        $sql = "INSERT INTO user ('CHANGE HERE') VALUES(?,?,?,?,?)";
        $stmt = $conn->prepare($sql);//prepare the sql for data

        $stmt->bindParam(1, $post["CHANGE HERE"]);
        //hash the password
        $hpswd = password_hash($post["CHANGE HERE"], PASSWORD_DEFAULT); /*hashes password using prebuilt library in php
        ...
        $stmt->bindParam(3, $post["CHANGE HERE"]);
        $stmt->bindParam(4, $post["CHANGE HERE"]);
        $stmt->bindParam(5, $post["CHANGE HERE"]);
...
function login($conn,$post){
    try{
        $sql = "SELECT CHANGE HERE FROM CHANGE HERE WHERE CHANGE HERE = ?"; //select everything from the user table where  = the entered
        $stmt = $conn->prepare($sql);//prepare the sql for data
...
function new_console($conn, $post)
{
    try {//we are doing a prepared statement where we send the statement and the data separately to prevent sql injections
        $sql = "INSERT INTO console (CHANGE HERE) VALUES(?, ?, ?, ?, ?)"; // inserts items into the specified fields, we leave them as question marks to allow data to be entered separately at a later date
        $stmt = $conn->prepare($sql);//prepare the sql for data

        $stmt->bindParam(1, $post['CHANGE HERE']);
        $stmt->bindParam(2, $post['CHANGE HERE']);
        $stmt->bindParam(3, $post['CHANGE HERE']); //we are binding the data from our form to an sql statment this makes it more secure from an sql attack and makes it unlikely for people to hijack an sql statement
        $stmt->bindParam(4, $post['CHANGE HERE']);
        $stmt->bindParam(5, $post['CHANGE HERE']);
...
{
    try {
        $sql = "SELECT CHANGE HERE FROM CHANGE HERE WHERE CHANGE HERE = ?"; //set up sql statement
